//************************************************************************//
// API "cellar" version 1.0: Application Media Types
//
...
func MarshalBottle(source *Bottle, inErr error) (target map[string]interface{}, err error) {
	...
	if err == nil {
		...
		links["account"], err = app.MarshalAccountLink(source.Account, err)
		target["links"] = links
	}
	return
}
...
func MarshalBottleFull(source *Bottle, inErr error) (target map[string]interface{}, err error) {
	...
	if source.Account != nil {
		tmp57["account"], err = app.MarshalAccount(source.Account, err)
	}
	target = tmp57
	if err == nil {
		links := make(map[string]interface{})
...
func UnmarshalBottle(source interface{}, inErr error) (target *Bottle, err error) {
	...
	if val, ok := source.(map[string]interface{}); ok {
		...
		if v, ok := val["account"]; ok {
			var tmp59 *app.Account
			tmp59, err = app.UnmarshalAccount(v, err)
			target.Account = tmp59
		}
		if v, ok := val["color"]; ok {
			...
			if err == nil {
				if !(tmp60 == "red" || tmp60 == "white" || tmp60 == "rose" || tmp60 == "yellow" || tmp60 == "sparkling") {
					err = goa.InvalidEnumValueError(`load.Color`, tmp60, []interface{}{"red", "white", "rose", "yellow", "sparkling"}, err)
				}
			}
			target.Color = &tmp60
		}
		if v, ok := val["country"]; ok {
			...
			if err == nil {
				if len(tmp61) < 2 {
					err = goa.InvalidLengthError(`load.Country`, tmp61, len(tmp61), 2, true, err)
				}
			}
			target.Country = &tmp61
		}
		if v, ok := val["created_at"]; ok {
			...
			if err == nil {
				if err2 := goa.ValidateFormat(goa.FormatDateTime, tmp62); err2 != nil {
					err = goa.InvalidFormatError(`load.CreatedAt`, tmp62, goa.FormatDateTime, err2, err)
				}
			}
			target.CreatedAt = &tmp62
		}
		if v, ok := val["href"]; ok {
			var tmp63 string
			if val, ok := v.(string); ok {
		...
		if v, ok := val["name"]; ok {
			...
			if err == nil {
				if len(tmp65) < 2 {
					err = goa.InvalidLengthError(`load.Name`, tmp65, len(tmp65), 2, true, err)
				}
			}
			target.Name = &tmp65
		}
		if v, ok := val["rating"]; ok {
			...
			if err == nil {
				if tmp66 < 1 {
					err = goa.InvalidRangeError(`load.Rating`, tmp66, 1, true, err)
				}
				if tmp66 > 5 {
					err = goa.InvalidRangeError(`load.Rating`, tmp66, 5, false, err)
				}
			}
			target.Rating = &tmp66
		}
		...
		if v, ok := val["review"]; ok {
			...
			if err == nil {
				if len(tmp68) < 10 {
					err = goa.InvalidLengthError(`load.Review`, tmp68, len(tmp68), 10, true, err)
				}
				if len(tmp68) > 300 {
					err = goa.InvalidLengthError(`load.Review`, tmp68, len(tmp68), 300, false, err)
				}
			}
			target.Review = &tmp68
		}
		if v, ok := val["sweetness"]; ok {
			...
			if err == nil {
				if tmp69 < 1 {
					err = goa.InvalidRangeError(`load.Sweetness`, tmp69, 1, true, err)
				}
				if tmp69 > 5 {
					err = goa.InvalidRangeError(`load.Sweetness`, tmp69, 5, false, err)
				}
			}
			target.Sweetness = &tmp69
		}
		if v, ok := val["updated_at"]; ok {
			...
			if err == nil {
				if err2 := goa.ValidateFormat(goa.FormatDateTime, tmp70); err2 != nil {
					err = goa.InvalidFormatError(`load.UpdatedAt`, tmp70, goa.FormatDateTime, err2, err)
				}
			}
		...
		if v, ok := val["varietal"]; ok {
			...
			if err == nil {
				if len(tmp71) < 4 {
					err = goa.InvalidLengthError(`load.Varietal`, tmp71, len(tmp71), 4, true, err)
				}
			}
			target.Varietal = &tmp71
		}
		if v, ok := val["vineyard"]; ok {
			...
			if err == nil {
				if len(tmp72) < 2 {
					err = goa.InvalidLengthError(`load.Vineyard`, tmp72, len(tmp72), 2, true, err)
				}
			}
			target.Vineyard = &tmp72
		}
		if v, ok := val["vintage"]; ok {
			...
			if err == nil {
				if tmp73 < 1900 {
					err = goa.InvalidRangeError(`load.Vintage`, tmp73, 1900, true, err)
				}
				if tmp73 > 2020 {
					err = goa.InvalidRangeError(`load.Vintage`, tmp73, 2020, false, err)
				}
			}
			target.Vintage = &tmp73
		}
	} else {
...
func (mt BottleCollection) Dump(view BottleCollectionViewEnum) (res []map[string]interface{}, err error) {
	if view == BottleCollectionDefaultView {
		...
		for i, tmp74 := range mt {
			var tmp75 map[string]interface{}
			tmp75, err = MarshalBottle(tmp74, err)
			res[i] = tmp75
		}
	}
	if view == BottleCollectionTinyView {
		...
		for i, tmp76 := range mt {
			...
			tmp77, err = MarshalBottleTiny(tmp76, err)
			res[i] = tmp77
		}
	}
	return
...
func UnmarshalBottleCollection(source interface{}, inErr error) (target BottleCollection, err error) {
	...
	if val, ok := source.([]interface{}); ok {
		...
		for tmp78, v := range val {
			target[tmp78], err = UnmarshalBottle(v, err)
		}
	} else {
